...
- Connection inputs:
  ...
  - `access_mode` (optional, default `READ_WRITE`) — `READ_WRITE` or `READ_ONLY`; use `READ_ONLY` for concurrent multi-process reads.
- DSN form: `duckdb:///{database_path}?access_mode={access_mode}` (e.g. `duckdb:///:memory:?access_mode=READ_WRITE`, `duckdb:////absolute/path/file.db?access_mode=READ_ONLY`).
...
- **Single writer.** A DuckDB file supports one read-write connection at a time; the connector uses `pool_size: 1`. Set `access_mode` to `READ_ONLY` for concurrent multi-process reads.
