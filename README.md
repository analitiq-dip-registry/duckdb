...
- **There is no read-only mode** — the connector always opens the database read-write, and the database path is the only connection setting. Earlier versions showed a "Read-only" checkbox, but it was inert: the DSN template interpolates only the database path, so the value never reached DuckDB. It would not have worked as spelled even if it had been appended to the URL — `duckdb_engine` forwards URL query parameters as DuckDB *config keys*, and DuckDB has no `read_only` config key, so `?read_only=true` fails with `Catalog Error: unrecognized configuration parameter "read_only"`. The setting that does open a database read-only is `access_mode=READ_ONLY`; it was not wired up because this connector is also a write destination, and opening read-only would break every write.
- **Single writer** — A DuckDB file supports only one read-write connection at a time, and the connector uses a single pooled connection (`pool_size: 1`). There is no supported way to attach to a file that another process holds open read-write.
